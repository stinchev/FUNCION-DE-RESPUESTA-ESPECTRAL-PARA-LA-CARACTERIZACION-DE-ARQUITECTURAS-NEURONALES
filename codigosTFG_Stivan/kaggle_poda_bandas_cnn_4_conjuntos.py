"""
Poda de bandas de la CNN (barrido progresivo en pasos del 5% + eliminacion
completa de banda baja/media/alta), extendida a los 4 datasets del TFG
(MNIST, Fashion-MNIST, KMNIST, CIFAR-10 en gris) en una sola ejecucion.

Es exactamente el mismo experimento del Capitulo 2 (Cuadro 2.2 y las curvas
de barrido progresivo), pero sin los autoencoders (no se piden aqui) y
recorriendo los 4 datasets internamente, en vez de una ejecucion por dataset
con --dataset como en kaggle_vision_cnn_autoencoder_experiment.py.

Dos analisis por dataset y semilla:
  1) Barrido progresivo: se va eliminando un porcentaje creciente (por
     defecto de 5 en 5%) de la banda alta, de la banda baja y de la banda
     media (por separado), midiendo la precision en cada paso.
  2) Eliminacion completa de cada banda fija (baja/media/alta, el criterio
     unico de bandas del TFG), midiendo la caida de precision resultante.
"""
...
# Umbrales de banda fijos del TFG, uno por tamano de imagen (mismo criterio:
# tercios aproximadamente iguales del espectro radial).
DATASET_SPECS = {
    ...
    "cifar10": {"display_name": "CIFAR-10 (gris)", "image_shape": (32, 32), "band_low_max": 10.3, "band_high_min": 14.8},
}
...
def parse_args(argv: list[str] | None = None) -> ExperimentConfig:
    parser = argparse.ArgumentParser(description="Poda de bandas de la CNN (barrido progresivo + eliminacion completa), 4 datasets.")
...
def default_output_root() -> str:
    if os.path.exists("/kaggle/working"):
        return "/kaggle/working/cnn_band_ablation_outputs"
    return "cnn_band_ablation_experiment/outputs"
...
# ---------------------------------------------------------------------------
# Mascaras de frecuencia (identicas a kaggle_vision_cnn_autoencoder_experiment.py)
# ---------------------------------------------------------------------------
...
def remove_media_frequencies(images: np.ndarray, remove_fraction: float, band_mid_rho: float):
    mask = media_frequency_keep_mask(images.shape[1], images.shape[2], remove_fraction, band_mid_rho)
    shifted = np.fft.fftshift(np.fft.fft2(images, axes=(-2, -1)), axes=(-2, -1))
    restored = np.fft.ifft2(np.fft.ifftshift(shifted * mask[np.newaxis], axes=(-2, -1)), axes=(-2, -1)).real
    return np.clip(restored, 0.0, 1.0), mask
...
def radial_frequency_grid_centered(height: int, width: int) -> np.ndarray:
    """Centrado en height//2, width//2 (igual que fftshift), para que el
    numero de modos coincida exactamente con el criterio de banda fijo."""
    yy, xx = np.indices((height, width))
...
def evaluate_ablation(model, x_test_img, y_test, ablation_step, mode, band_mid_rho):
    ...
    for remove_fraction in np.arange(0.0, 0.951, ablation_step):
        ...
        if mode == "media":
            degraded, mask = remove_media_frequencies(x_test_img, remove_fraction, band_mid_rho)
        else:
...
def run_dataset(dataset: str, cfg: ExperimentConfig) -> dict[str, Any]:
    ...
    for seed in cfg.seeds:
        ...
        print(f"  [seed={seed}] accuracy base = {cnn_accuracy*100:.2f}%")
        ...
        media_rows = evaluate_ablation(cnn, data["x_test_img"], data["y_test"], cfg.ablation_step, "media", band_mid_rho)
...
def save_ablation_plot(rows_per_seed: list[list[dict]], output_path: Path, mode: str, display_name: str) -> None:
    ...
    plt.ylabel("Precisión de la CNN en test (%)")
    plt.title(f"Precisión al eliminar {MODE_LABEL_ES[mode]} — {display_name}")
...
def save_band_ablation_plot(band_rows_per_seed: list[list[dict]], output_path: Path, display_name: str) -> None:
    ...
    plt.ylabel("Precisión de la CNN en test (%)")
    plt.title(f"Precisión al eliminar por completo cada banda — {display_name}")
...
def build_report(output_dir: Path, cfg: ExperimentConfig, results: list[dict[str, Any]]) -> None:
    ...
    for result in results:
        ...
        lines += [
            ...
            f"- Accuracy base: `{agg['cnn_test_accuracy']['mean']*100:.2f}% ± {agg['cnn_test_accuracy']['std']*100:.2f}pp`",
            "",
...
def main() -> None:
    ...
    for result in results:
        agg = result["aggregate"]
        print(f"{result['display_name']}: accuracy_base={agg['cnn_test_accuracy']['mean']*100:.2f}%±{agg['cnn_test_accuracy']['std']*100:.2f}pp")
        for band in ("baja", "media", "alta"):
            band_res = agg["cnn_band_ablation"][band]
