"""
Mismo experimento que kaggle_cnn_band_ablation_4datasets.py (barrido
progresivo en pasos del 5% + eliminacion completa de banda baja/media/alta),
pero con ResNet-18 preentrenada en ImageNet y con el backbone completamente
congelado (solo se entrena una cabeza lineal nueva de 10 clases), para los
4 datasets del TFG en una sola ejecucion.

Mismas decisiones de diseno que en kaggle_vision_resnet18_spectral_cdf_experiment.py:
- PyTorch (Keras/TensorFlow no trae ResNet-18 preentrenada de fabrica).
- Backbone congelado, cabeza lineal entrenada (linear probe).
- La reconstruccion por FFT se hace siempre en la resolucion nativa de cada
  dataset; el redimensionado a 224x224 y la replicacion a 3 canales se hacen
  DESPUES, justo antes de pasar por la red congelada.
- El barrido progresivo se evalua sobre un subconjunto fijo de test
  (--eval-subset-size, por defecto 2000) por coste computacional, ya que aqui
  no se pueden cachear features (cada paso cambia la imagen de entrada). La
  eliminacion completa de banda (solo 3 evaluaciones por semilla) SI usa el
  test completo, por ser barata.
"""
...
MODE_LABEL_ES = {"high": "frecuencias altas", "low": "frecuencias bajas", "media": "frecuencias medias"}
BAND_LABEL_ES = {"baja": "banda baja", "media": "banda media", "alta": "banda alta"}
...
def parse_args(argv: list[str] | None = None) -> ExperimentConfig:
    parser = argparse.ArgumentParser(description="Poda de bandas con ResNet-18 congelada (barrido progresivo + eliminacion completa), 4 datasets.")
    parser.add_argument("--datasets", type=str, nargs="+", default=list(DATASET_ORDER), choices=sorted(DATASET_SPECS.keys()))
    parser.add_argument("--seeds", type=int, nargs="+", default=[42, 43, 44])
    parser.add_argument("--eval-subset-size", type=int, default=2000, help="-1 para usar el test completo en el barrido progresivo")
...
def default_output_root() -> str:
    if os.path.exists("/kaggle/working"):
        return "/kaggle/working/resnet18_band_ablation_outputs"
    return "resnet18_band_ablation_experiment/outputs"
...
# ---------------------------------------------------------------------------
# ResNet-18 congelada + cabeza lineal
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Mascaras de frecuencia (identicas al resto del TFG)
# ---------------------------------------------------------------------------
...
def media_frequency_keep_mask(height: int, width: int, remove_fraction: float, band_mid_rho: float) -> np.ndarray:
    radius = radial_distance_map(height, width)
    distance_to_media_center = np.abs(radius - band_mid_rho)
    flat_order = np.argsort(distance_to_media_center, axis=None)
    remove_count = min(max(int(round(remove_fraction * height * width)), 0), height * width - 1)
    keep_mask = np.ones(height * width, dtype=bool)
    if remove_count > 0:
        keep_mask[flat_order[:remove_count]] = False
...
def band_keep_mask(height: int, width: int, band: str, band_low_max: float, band_high_min: float) -> np.ndarray:
    ...
    if band == "baja":
        ...
        raise ValueError(f"banda desconocida: {band}")
    return ~remove
...
def evaluate_ablation(backbone, head, x_test_img, y_test, ablation_step, mode, band_mid_rho, device, batch_size):
    ...
    for remove_fraction in np.arange(0.0, 0.951, ablation_step):
        ...
        if mode == "high":
            ...
            mask = media_frequency_keep_mask(h, w, remove_fraction, band_mid_rho)
        degraded = apply_mask(x_test_img, mask)
        accuracy = evaluate_full_pipeline(backbone, head, degraded, y_test, device, batch_size)
...
def run_dataset(dataset: str, cfg: ExperimentConfig, backbone: nn.Module, device: torch.device) -> dict[str, Any]:
    ...
    for seed in cfg.seeds:
        print(f"  [seed={seed}] extrayendo features y entrenando cabeza lineal...")
        set_seed(seed)
        ...
        print(f"  [seed={seed}] val_accuracy={val_acc*100:.2f}% ; accuracy base test completo={baseline_acc*100:.2f}%")
        ...
        media_rows = evaluate_ablation(backbone, head, x_sweep, y_sweep, cfg.ablation_step, "media", band_mid_rho, device, cfg.batch_size)
        ...
        per_seed_results.append({
            ...
            "media_frequency_ablation": media_rows, "band_ablation": band_rows,
        })
...
def aggregate_seeds(per_seed_results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    agg["band_ablation"] = {}
    for band_idx, band in enumerate(("baja", "media", "alta")):
...
def save_ablation_plot(rows_per_seed: list[list[dict]], output_path: Path, mode: str, display_name: str) -> None:
    ...
    for seed_idx, rows in enumerate(rows_per_seed):
        ...
        plt.plot(x, y, marker="o", alpha=0.6, label=f"semilla {seed_idx}")
    plt.xlabel(f"Coeficientes de {MODE_LABEL_ES[mode]} eliminados (%)")
    plt.ylabel("Precisión en test (%)")
    plt.title(f"ResNet-18 congelada: precisión al eliminar {MODE_LABEL_ES[mode]} — {display_name}")
...
def save_band_ablation_plot(band_rows_per_seed: list[list[dict]], output_path: Path, display_name: str) -> None:
    ...
    plt.bar([BAND_LABEL_ES[b] for b in bands], means, yerr=stds, capsize=6, color=["#4c72b0", "#55a868", "#c44e52"])
    plt.ylabel("Precisión en test (%)")
    plt.title(f"ResNet-18 congelada: precisión al eliminar cada banda — {display_name}")
...
def build_report(output_dir: Path, cfg: ExperimentConfig, results: list[dict[str, Any]]) -> None:
    lines = [
        ...
        f"- Subconjunto de test para el barrido progresivo: `{cfg.eval_subset_size if cfg.eval_subset_size > 0 else 'completo'}`",
        "",
    ]
    for result in results:
        ...
        lines += [
            ...
            f"- Accuracy base (test completo): `{agg['test_accuracy']['mean']*100:.2f}% ± {agg['test_accuracy']['std']*100:.2f}pp`",
            "",
...
def main() -> None:
    ...
    if device.type == "cpu":
        print("AVISO: ejecutando en CPU. Con ResNet-18 a 224x224 y varios datasets esto será muy lento; usa --quick.")
    ...
    for dataset in cfg.datasets:
        ...
        save_ablation_plot([r["media_frequency_ablation"] for r in result["per_seed_results"]],
                            output_dir / f"{dataset}_media_ablation_curve.png", "media", result["display_name"])
    ...
    for result in results:
        agg = result["aggregate"]
        print(f"{result['display_name']}: accuracy_base={agg['test_accuracy']['mean']*100:.2f}%±{agg['test_accuracy']['std']*100:.2f}pp")
        for band in ("baja", "media", "alta"):
            band_res = agg["band_ablation"][band]
            print(f"  banda {band}: accuracy={band_res['accuracy']['mean']*100:.2f}%±{band_res['accuracy']['std']*100:.2f}pp "
                  f"caída={band_res['accuracy_drop_from_baseline_pp']['mean']:.2f}pp")
