"""
Anexo exploratorio: SRF de un Autoencoder (AE) usado como extractor de
características para clasificación en su espacio latente.

Este script NO forma parte del análisis comparativo riguroso de los
Capítulos 2 y 3 del TFG (CNN entrenada desde cero vs. ResNet-18 preentrenada
y congelada). Es un experimento exploratorio, a mano alzada, para comprobar
de forma rápida si la SRF de un AE se comporta de manera distinta a las dos
arquitecturas ya estudiadas. La razón para no integrarlo en el cuerpo
principal del trabajo es que un AE introduce dos ejes de variación que no
existen en la CNN ni en la ResNet-18: el objetivo de reconstrucción (no solo
de clasificación) y la dimensión del espacio latente. Mezclar una
arquitectura de naturaleza tan distinta con la comparación principal
ensuciaría la discusión; se deja aquí como un avance de trabajo futuro, no
como una conclusión firme del TFG.

Metodología (idéntica en su núcleo a la del resto del TFG, para que la SRF
resultante sea comparable en su definición, aunque no en su rigor
experimental):
  1. Se entrena un autoencoder convolucional de forma no supervisada
     (pérdida de reconstrucción MSE) sobre el conjunto de entrenamiento.
  2. Se congela el encoder y se entrena un clasificador lineal (softmax)
     directamente sobre el código latente z, usando las etiquetas de
     entrenamiento/validación.
  3. Se realiza el mismo barrido espectral acumulativo que en el resto del
     TFG (incorporando modos de Fourier de radio creciente): cada imagen
     degradada se codifica con el encoder congelado y se clasifica con la
     cabeza lineal congelada, evaluando la precisión en cada paso.
  4. Se ajusta la curva a monótona mediante regresión isotónica y se
     calculan los mismos estadísticos que en el Capítulo 3: E[R], sigma[R],
     mediana, moda, AUC, entropía H y hueco de entropía (negentropía) ΔH.

Por defecto se ejecuta solo sobre MNIST y con pocas semillas, precisamente
para mantener el experimento ligero y exploratorio.

Uso en Kaggle:
    python kaggle_srf_autoencoder_experiment.py
    python kaggle_srf_autoencoder_experiment.py --datasets mnist fashion_mnist
    python kaggle_srf_autoencoder_experiment.py --latent-dim 64
    python kaggle_srf_autoencoder_experiment.py --quick   # prueba rapida local
"""
...
# ---------------------------------------------------------------------------
# Datasets (mismos loaders que en el resto del TFG)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Autoencoder convolucional (misma estructura de conv/pooling que la CNN del
# resto del TFG hasta el cuello de botella, para que la comparacion "6272 ->
# latent_dim" vs. "6272 -> 128" de la CNN clasificadora sea lo mas directa
# posible)
# ---------------------------------------------------------------------------
...
def cdf_statistics(normalized_radius: np.ndarray, cdf_values: np.ndarray) -> dict[str, Any]:
    ...
    entropia (negentropia), calculados sobre la SRF F(r) tal y como se
