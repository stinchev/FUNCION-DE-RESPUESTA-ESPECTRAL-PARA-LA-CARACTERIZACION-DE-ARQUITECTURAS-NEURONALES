"""
Experimento complementario 2: efecto de la FUNCIÓN DE ACTIVACIÓN sobre la SRF,
con la ResNet-18 preentrenada y congelada incluida como CURVA DE REFERENCIA.

Idea principal: mantener fija la arquitectura (mismo número de capas, mismos
filtros, mismo optimizador, mismas épocas, misma partición de datos, mismas
semillas) y mismo dataset (por defecto MNIST), cambiando únicamente la función
de activación de las capas ocultas de la CNN: ReLU, GELU, tanh o sigmoide. Si
dos redes con precisión final parecida muestran SRF distintas, la forma de la
respuesta espectral no depende solo del rendimiento final, sino también de la
activación elegida -en línea con lo ya discutido en el Capítulo 1 sobre
saturación y desvanecimiento del gradiente con sigmoide/tanh frente a ReLU
(Glorot y Bengio 2010; Nair y Hinton 2010)-.

La ResNet-18 preentrenada tiene el backbone COMPLETAMENTE CONGELADO (pesos de
ImageNet, activación ReLU ya fija dentro de esos pesos): no tiene sentido
"cambiarle la activación" sin reentrenar todo el backbone desde cero, lo que
contradice el propio planteamiento de "linear probe" usado en el resto del
TFG. Por eso aquí NO se hace variar su activación: se entrena una única vez
por semilla (igual que en el resto del TFG) y su SRF se añade como curva de
REFERENCIA fija, para poder comparar visualmente si alguna de las variantes de
activación de la CNN se acerca más o menos a su comportamiento espectral.

La activación de la capa de salida de la CNN se mantiene siempre en softmax
(10 clases); solo se varía la activación de las capas ocultas (tres bloques
convolucionales + la capa densa de 128 unidades).

Este script:
  1. Para cada activación en {relu, gelu, tanh, sigmoid} y cada semilla,
     entrena la misma CNN (TensorFlow/Keras) sobre el dataset elegido.
  2. Entrena además, con las mismas semillas, una cabeza lineal sobre la
     ResNet-18 preentrenada y congelada (PyTorch/torchvision), como en
     kaggle_vision_resnet18_spectral_cdf_experiment.py.
  3. Calcula la SRF de cada red (CNN por activación, y ResNet-18) mediante el
     mismo barrido acumulativo de modos de Fourier + corrección isotónica que
     en el resto del TFG.
  4. Calcula, por semilla, los estadísticos derivados de la SRF: E[R],
     sigma[R], mediana, moda, área bajo la curva, entropía H y hueco de
     entropía (negentropía) Delta H; los agrega (media +/- desviación típica)
     entre semillas para cada variante.
  5. Genera una figura comparando las curvas SRF medias de las cuatro
     activaciones de la CNN junto con la curva de referencia de la ResNet-18
     (en línea discontinua, para distinguirla visualmente), y un informe con
     la tabla de estadísticos.

Uso en Kaggle:
    python kaggle_srf_activation_experiment.py --dataset mnist --seeds 42 43 44
    python kaggle_srf_activation_experiment.py --activations relu tanh sigmoid gelu
    python kaggle_srf_activation_experiment.py --no-resnet18   # solo las 4 activaciones de la CNN
    python kaggle_srf_activation_experiment.py --quick          # prueba rapida local
"""
...
# ---------------------------------------------------------------------------
# Datasets (los mismos loaders que en el resto del TFG; sirven tanto para la
# CNN como para la ResNet-18, garantizando idéntica partición por semilla)
# ---------------------------------------------------------------------------
...
ACTIVATION_DISPLAY = {"relu": "ReLU", "gelu": "GELU", "tanh": "tanh", "sigmoid": "sigmoide"}
RESNET_LABEL = "ResNet-18 (referencia, backbone congelado)"
...
class ExperimentConfig:
    ...
    # ResNet-18 (PyTorch/torchvision), solo si include_resnet18=True
    ...
    eval_subset_size: int = 2000  # solo limita el barrido de la ResNet-18, por coste
    output_root: str = ""
    quick: bool = False


def parse_args(argv: list[str] | None = None) -> ExperimentConfig:
    parser = argparse.ArgumentParser(description="Efecto de la funcion de activacion sobre la SRF de la CNN, con ResNet-18 como referencia.")
    ...
    parser.add_argument("--no-resnet18", action="store_true", help="No entrenar/mostrar la curva de referencia de ResNet-18")
    ...
    parser.add_argument("--eval-subset-size", type=int, default=2000, help="-1 para usar el test completo en el barrido de la ResNet-18")
...
def default_output_root() -> str:
    if os.path.exists("/kaggle/working"):
        return "/kaggle/working/srf_activation_outputs"
    return "srf_activation_experiment/outputs"
...
# ---------------------------------------------------------------------------
# CNN (misma arquitectura que en el resto del TFG; solo cambia la activación
# de las capas ocultas -- la capa de salida sigue siendo softmax siempre)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# ResNet-18 congelada + cabeza lineal entrenable (PyTorch/torchvision, mismo
# pipeline que kaggle_vision_resnet18_spectral_cdf_experiment.py). Se entrena
# una única vez por semilla, SIN variar activación (ver docstring del módulo).
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Barrido espectral acumulativo + CDF + estadísticos (idéntico al resto del TFG)
# ---------------------------------------------------------------------------
...
def run_seed_cnn(dataset: str, cfg: ExperimentConfig, activation: str, seed: int) -> dict[str, Any]:
    ...
    print(f"  [activation={activation} seed={seed}] entrenando CNN...")
    set_seed(seed)
    ...
    print(f"  [activation={activation} seed={seed}] accuracy base = {baseline_acc*100:.2f}% ; barriendo espectro...")
...
def run_seed_resnet(dataset: str, cfg: ExperimentConfig, seed: int, backbone: nn.Module, device: torch.device) -> dict[str, Any]:
    ...
    print(f"  [ResNet-18 seed={seed}] val_accuracy={val_acc*100:.2f}% ; accuracy base test completo={baseline_acc*100:.2f}% ; barriendo espectro...")
...
def build_report(output_dir: Path, cfg: ExperimentConfig, results: list[dict[str, Any]]) -> None:
    ...
    lines = [
        f"# Efecto de la función de activación sobre la SRF de la CNN, vs. ResNet-18 — {dataset_display}",
        "",
        "## Setup",
        f"- Dataset: `{dataset_display}`",
        f"- Activaciones comparadas en la CNN: `{cfg.activations}` (capas ocultas; la capa de salida es siempre softmax)",
        f"- ResNet-18 de referencia incluida: `{cfg.include_resnet18}` (backbone congelado, activación fija; no varía por diseño)",
...
def main() -> None:
    ...
    dataset_display = DATASET_SPECS[cfg.dataset]["display_name"]
    print(f"Dataset: {dataset_display}  |  Activaciones CNN: {cfg.activations}  |  ResNet-18 referencia: {cfg.include_resnet18}  |  Semillas: {cfg.seeds}")
    print(f"Output directory: {output_dir.resolve()}")

    results = []
    for activation in cfg.activations:
        print(f"\n===== ACTIVACIÓN CNN: {ACTIVATION_DISPLAY[activation]} =====")
