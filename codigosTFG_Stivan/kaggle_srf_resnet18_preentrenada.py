"""
Barrido espectral acumulativo + interpretación como CDF (misma idea que
kaggle_vision_spectral_cdf_experiment.py) pero usando ResNet-18 preentrenada en
ImageNet como clasificador, en vez de la CNN pequeña entrenada desde cero.

Decisiones acordadas expresamente para este script:

- FRAMEWORK: PyTorch + torchvision.models.resnet18 (pesos IMAGENET1K_V1).
  Keras/TensorFlow no trae una ResNet-18 preentrenada de fábrica (solo ResNet50
  y mayores en keras.applications), así que este es el único script del TFG en
  PyTorch; el resto sigue en TensorFlow/Keras. Son experimentos independientes.

- TRANSFERENCIA: el backbone queda COMPLETAMENTE CONGELADO; solo se entrena una
  cabeza lineal nueva de 10 clases sobre las 512 features de salida del
  backbone (linear probe). Mide la sensibilidad espectral de las features de
  ImageNet "puras", sin adaptarlas al dominio de cada dataset.

- ENTRADA: la reconstrucción por FFT (quitar/incorporar modos) se hace siempre
  en la resolución NATIVA de cada dataset (28x28 o 32x32, igual que en el resto
  del TFG). Solo DESPUÉS de reconstruir la imagen se redimensiona a 224x224
  (tamaño nativo de ImageNet) y se replica a 3 canales para dársela a la red.

- PRESUPUESTO DE CÓMPUTO: se mantienen 3 semillas y ~150 puntos por curva,
  igual que el resto del TFG. Como aquí NO se pueden cachear features del
  backbone durante el barrido (cada paso cambia la imagen de entrada, así que
  hace falta un forward completo de ResNet-18 en cada uno de los ~150 pasos),
  el barrido evalúa sobre un subconjunto fijo del test (`--eval-subset-size`,
  por defecto 2000 imágenes; usar -1 para el test completo) en vez de las
  ~10000 imágenes completas. El entrenamiento de la cabeza sí usa el
  train/val completos, y la precisión base (`baseline_accuracy`) se calcula
  siempre sobre el test completo.

Requiere GPU para ser viable en un tiempo razonable: ResNet-18 a 224x224 es
mucho más pesada que la CNN pequeña del otro script, y aquí se hace un forward
completo por cada punto del barrido.
"""
...
def parse_args(argv: list[str] | None = None) -> ExperimentConfig:
    parser = argparse.ArgumentParser(description="Barrido espectral acumulativo + CDF con ResNet-18 preentrenada (backbone congelado).")
    ...
    parser.add_argument("--eval-subset-size", type=int, default=2000, help="-1 para usar el test completo en el barrido")
...
def default_output_root() -> str:
    if os.path.exists("/kaggle/working"):
        return "/kaggle/working/resnet18_spectral_cdf_outputs"
    return "resnet18_spectral_cdf_experiment/outputs"
...
# ---------------------------------------------------------------------------
# Split train/val/test estratificado (idéntico criterio al resto del TFG)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# ResNet-18 congelada + cabeza lineal entrenable
# ---------------------------------------------------------------------------
...
def build_frozen_resnet18(device: torch.device) -> nn.Module:
    try:
        ...
        # torchvision < 0.13 no tiene el enum `weights`; API antigua de respaldo.
        backbone = torchvision.models.resnet18(pretrained=True)
    backbone.fc = nn.Identity()  # deja las 512 features de salida del pooling global, sin la cabeza de 1000 clases de ImageNet
...
def preprocess_for_resnet(images_hw: np.ndarray, device: torch.device) -> torch.Tensor:
    """images_hw: (N,H,W) float32 en [0,1], en la resolución NATIVA del dataset
    (ya reconstruida por FFT si procede). Redimensiona a 224x224, replica a 3
    canales y normaliza con la media/desviación de ImageNet."""
...
def pool_adjacent_violators(y: np.ndarray) -> np.ndarray:
    ...
    violators, Barlow et al. 1972). Los puntos vienen ordenados por `x`
    creciente (orden del propio barrido), así que basta operar sobre `y`."""
    values: list[float] = []
    weights: list[float] = []
    for yi in y:
...
def cdf_statistics(normalized_radius: np.ndarray, cdf_values: np.ndarray) -> dict[str, float]:
    """E[X] = int_0^1 (1-F(r)) dr ; E[X^2] = 2 * int_0^1 r (1-F(r)) dr."""
...
def main() -> None:
    ...
    if device.type == "cuda":
        try:
            _ = torch.zeros(1, device=device) + 1.0  # prueba mínima de compatibilidad GPU/build de PyTorch
